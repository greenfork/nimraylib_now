...
void DrawBillboardRec(Camera camera, Texture2D texture, NmrlbNow_Rectangle source, Vector3 center, float size, Color tint)
{
...
static Image LoadImageFromCgltfImage(cgltf_image *image, const char *texPath, Color tint)
{
    ...
    if (image->uri)
    {
        ...
        {
            rimage = NmrlbNow_LoadImage(TextFormat("%s/%s", texPath, image->uri));
