...
extern void LoadFontDefault(void)
{
    ...
    // NOTE: This memory should be freed at end! --> NmrlbNow_CloseWindow()
    defaultFont.chars = (CharInfo *)RL_MALLOC(defaultFont.charsCount*sizeof(CharInfo));
    defaultFont.recs = (NmrlbNow_Rectangle *)RL_MALLOC(defaultFont.charsCount*sizeof(NmrlbNow_Rectangle));
...
Font LoadFontFromImage(Image image, Color key, int firstChar)
{
    ...
    NmrlbNow_Rectangle tempCharRecs[MAX_GLYPHS_FROM_IMAGE];
    ...
    font.recs = (NmrlbNow_Rectangle *)RL_MALLOC(font.charsCount*sizeof(NmrlbNow_Rectangle));
...
#if defined(SUPPORT_FILEFORMAT_TTF)
Image GenImageFontAtlas(const CharInfo *chars, NmrlbNow_Rectangle **charRecs, int charsCount, int fontSize, int padding, int packMethod)
{
    Image atlas = { 0 };
    ...
    *charRecs = NULL;
    ...
    NmrlbNow_Rectangle *recs = (NmrlbNow_Rectangle *)RL_MALLOC(charsCount*sizeof(NmrlbNow_Rectangle));
...
void DrawTextRec(Font font, const char *text, NmrlbNow_Rectangle rec, float fontSize, float spacing, bool wordWrap, Color tint)
{
    DrawTextRecEx(font, text, rec, fontSize, spacing, wordWrap, tint, 0, 0, WHITE, WHITE);
}

// Draw text using font inside rectangle limits with support for text selection
void DrawTextRecEx(Font font, const char *text, NmrlbNow_Rectangle rec, float fontSize, float spacing, bool wordWrap, Color tint, int selectStart, int selectLength, Color selectTint, Color selectBackTint)
{
    int length = TextLength(text);  // Total length in bytes of the text, scanned by codepoints in loop
    ...
    for (int i = 0, k = 0; i < length; i++, k++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    DrawRectangleRec((NmrlbNow_Rectangle){ rec.x + textOffsetX - 1, rec.y + textOffsetY, (float)glyphWidth, (float)font.baseSize*scaleFactor }, selectBackTint);
...
void DrawTextCodepoint(Font font, int codepoint, Vector2 position, float fontSize, Color tint)
{
    ...
    NmrlbNow_Rectangle dstRec = { position.x + font.chars[index].offsetX*scaleFactor - (float)font.charsPadding*scaleFactor,
    ...
    NmrlbNow_Rectangle srcRec = { font.recs[index].x - (float)font.charsPadding, font.recs[index].y - (float)font.charsPadding,
...
static Font LoadBMFont(const char *fileName)
{
    ...
    font.recs = (NmrlbNow_Rectangle *)RL_MALLOC(charsCount*sizeof(NmrlbNow_Rectangle));
