...
void DrawCubeTextureRec(Texture2D texture, NmrlbNow_Rectangle source, Vector3 position, float width, float height, float length, Color color)
{
    float x = position.x;
    float y = position.y;
    float z = position.z;
...
void DrawBillboardRec(Camera camera, Texture2D texture, NmrlbNow_Rectangle source, Vector3 position, Vector2 size, Color tint)
{
...
void DrawBillboardPro(Camera camera, Texture2D texture, NmrlbNow_Rectangle source, Vector3 position, Vector3 up, Vector2 size, Vector2 origin, float rotation, Color tint)
{
...
static Image LoadImageFromCgltfImage(cgltf_image *cgltfImage, const char *texPath)
{
    ...
    if (cgltfImage->uri != NULL)     // Check if image data is provided as a uri (base64 or path)
    {
        ...
        {
            image = NmrlbNow_LoadImage(TextFormat("%s/%s", texPath, cgltfImage->uri));
        }
    }
