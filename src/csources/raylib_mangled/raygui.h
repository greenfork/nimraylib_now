...
RAYGUIAPI bool GuiWindowBox(NmrlbNow_Rectangle bounds, const char *title);                                       // Window Box control, shows a window that can be closed
RAYGUIAPI void GuiGroupBox(NmrlbNow_Rectangle bounds, const char *text);                                         // Group Box control with text name
RAYGUIAPI void GuiLine(NmrlbNow_Rectangle bounds, const char *text);                                             // Line separator control, could contain text
RAYGUIAPI void GuiPanel(NmrlbNow_Rectangle bounds);                                                              // Panel control, useful to group controls
RAYGUIAPI NmrlbNow_Rectangle GuiScrollPanel(NmrlbNow_Rectangle bounds, NmrlbNow_Rectangle content, Vector2 *scroll);               // Scroll Panel control

// Basic controls set
RAYGUIAPI void GuiLabel(NmrlbNow_Rectangle bounds, const char *text);                                            // Label control, shows text
RAYGUIAPI bool GuiButton(NmrlbNow_Rectangle bounds, const char *text);                                           // Button control, returns true when clicked
RAYGUIAPI bool GuiLabelButton(NmrlbNow_Rectangle bounds, const char *text);                                      // Label button control, show true when clicked
RAYGUIAPI bool GuiToggle(NmrlbNow_Rectangle bounds, const char *text, bool active);                              // Toggle Button control, returns true when active
RAYGUIAPI int GuiToggleGroup(NmrlbNow_Rectangle bounds, const char *text, int active);                           // Toggle Group control, returns active toggle index
RAYGUIAPI bool GuiCheckBox(NmrlbNow_Rectangle bounds, const char *text, bool checked);                           // Check Box control, returns true when active
RAYGUIAPI int GuiComboBox(NmrlbNow_Rectangle bounds, const char *text, int active);                              // Combo Box control, returns selected item index
RAYGUIAPI bool GuiDropdownBox(NmrlbNow_Rectangle bounds, const char *text, int *active, bool editMode);          // Dropdown Box control, returns selected item
RAYGUIAPI bool GuiSpinner(NmrlbNow_Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode);     // Spinner control, returns selected value
RAYGUIAPI bool GuiValueBox(NmrlbNow_Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode);    // Value Box control, updates input text with numbers
RAYGUIAPI bool GuiTextBox(NmrlbNow_Rectangle bounds, char *text, int textSize, bool editMode);                   // Text Box control, updates input text
RAYGUIAPI bool GuiTextBoxMulti(NmrlbNow_Rectangle bounds, char *text, int textSize, bool editMode);              // Text Box control with multiple lines
RAYGUIAPI float GuiSlider(NmrlbNow_Rectangle bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue);       // Slider control, returns selected value
RAYGUIAPI float GuiSliderBar(NmrlbNow_Rectangle bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue);    // Slider Bar control, returns selected value
RAYGUIAPI float GuiProgressBar(NmrlbNow_Rectangle bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue);  // Progress Bar control, shows current progress value
RAYGUIAPI void GuiStatusBar(NmrlbNow_Rectangle bounds, const char *text);                                        // Status Bar control, shows info text
RAYGUIAPI void GuiDummyRec(NmrlbNow_Rectangle bounds, const char *text);                                         // Dummy control for placeholders
RAYGUIAPI int GuiScrollBar(NmrlbNow_Rectangle bounds, int value, int minValue, int maxValue);                    // Scroll Bar control
RAYGUIAPI Vector2 GuiGrid(NmrlbNow_Rectangle bounds, float spacing, int subdivs);                                // Grid control


// Advance controls set
RAYGUIAPI int GuiListView(NmrlbNow_Rectangle bounds, const char *text, int *scrollIndex, int active);            // List View control, returns selected list item index
RAYGUIAPI int GuiListViewEx(NmrlbNow_Rectangle bounds, const char **text, int count, int *focus, int *scrollIndex, int active);      // List View with extended parameters
RAYGUIAPI int GuiMessageBox(NmrlbNow_Rectangle bounds, const char *title, const char *message, const char *buttons);                 // Message Box control, displays a message
RAYGUIAPI int GuiTextInputBox(NmrlbNow_Rectangle bounds, const char *title, const char *message, const char *buttons, char *text);   // Text Input Box control, ask for text
RAYGUIAPI Color GuiColorPicker(NmrlbNow_Rectangle bounds, Color color);                                          // Color Picker control (multiple color controls)
RAYGUIAPI Color GuiColorPanel(NmrlbNow_Rectangle bounds, Color color);                                           // Color Panel control
RAYGUIAPI float GuiColorBarAlpha(NmrlbNow_Rectangle bounds, float alpha);                                        // Color Bar Alpha control
RAYGUIAPI float GuiColorBarHue(NmrlbNow_Rectangle bounds, float value);                                          // Color Bar Hue control
...
static void SetShapesTexture(Texture2D tex, NmrlbNow_Rectangle rec); // -- GuiLoadStyle()
...
#else
    GuiDrawText("#120#", RAYGUI_CLITERAL(NmrlbNow_Rectangle){ bounds.x + bounds.width - GuiGetStyle(DROPDOWNBOX, ARROW_PADDING), bounds.y + bounds.height/2 - 6, 10, 10 },
...
bool GuiValueBox(NmrlbNow_Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    if (editMode)
    {
        ...
        NmrlbNow_Rectangle cursor = { bounds.x + GetTextWidth(textValue)/2 + bounds.width/2 + 2, bounds.y + 2*GuiGetStyle(VALUEBOX, BORDER_WIDTH), 4, bounds.height - 4*GuiGetStyle(VALUEBOX, BORDER_WIDTH) };
...
bool GuiTextBoxMulti(NmrlbNow_Rectangle bounds, char *text, int textSize, bool editMode)
{
    ...
    NmrlbNow_Rectangle cursor = { 0, -1, 4, (float)GuiGetStyle(DEFAULT, TEXT_SIZE) + 2 };
    ...
    for (int i = 0, codepointLength = 0; text[i] != '\0'; i += codepointLength)
    {
        ...
        NmrlbNow_Rectangle atlasRec = guiFont.recs[index];
...
int GuiScrollBar(NmrlbNow_Rectangle bounds, int value, int minValue, int maxValue)
{
    ...
    {
#if defined(RAYGUI_NO_RICONS)
        GuiDrawText(isVertical? "^" : "<", RAYGUI_CLITERAL(NmrlbNow_Rectangle){ arrowUpLeft.x, arrowUpLeft.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
                    GUI_TEXT_ALIGN_CENTER, Fade(GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))), guiAlpha));
        GuiDrawText(isVertical? "v" : ">", RAYGUI_CLITERAL(NmrlbNow_Rectangle){ arrowDownRight.x, arrowDownRight.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
                    GUI_TEXT_ALIGN_CENTER, Fade(GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))), guiAlpha));
#else
        GuiDrawText(isVertical? "#121#" : "#118#", RAYGUI_CLITERAL(NmrlbNow_Rectangle){ arrowUpLeft.x, arrowUpLeft.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
        ...
        GuiDrawText(isVertical? "#120#" : "#119#", RAYGUI_CLITERAL(NmrlbNow_Rectangle){ arrowDownRight.x, arrowDownRight.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
...
float GuiColorBarAlpha(NmrlbNow_Rectangle bounds, float alpha)
{
    ...
    GuiControlState state = guiState;
    NmrlbNow_Rectangle selector = { (float)bounds.x + alpha*bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT)/2, (float)bounds.y - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT), (float)bounds.height + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2 };
...
{
    GuiControlState state = guiState;
    NmrlbNow_Rectangle selector = { (float)bounds.x - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)bounds.y + hue/360.0f*bounds.height - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT)/2, (float)bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2, (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT) };
    ...
    if (tryBinary)
    {
            ...
            (signature[3] == ' '))
        {
            ...
            if (fontDataSize > 0)
            {
                ...
                // Load font recs data
                font.recs = (NmrlbNow_Rectangle *)RAYGUI_CALLOC(font.glyphCount, sizeof(NmrlbNow_Rectangle));
                for (int i = 0; i < font.glyphCount; i++) fread(&font.recs[i], 1, sizeof(NmrlbNow_Rectangle), rgsFile);
